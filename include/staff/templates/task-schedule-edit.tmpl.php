...
if (!$info['title'])
    $info['title'] = sprintf(__('%s Tarea Programada #%s'),
            __('Edit'), str_pad($task->getId(), 6, "0", STR_PAD_LEFT)
            );

$action = $info['action'] ?: ('#task-schedule/'.$task->getId().'/edit');

$namespace = sprintf('schedule.%d.edit', $task->getId());
...
<form method="post" class="task" action="<?php echo $action; ?>">
    ...
    if ($forms) {
        foreach ($forms as $form)
            echo $form->getForm(false, array('mode' => 'edit'))->asTable(
                    __('Información de Tarea Programada'),
                    array(
    ...
    <div>
        <textarea class="richtext no-bar" name="note" cols="21" rows="6"
            style="width:80%;"
            placeholder="<?php echo 'Razón de la edición de la tarea programada (opcional)'; ?>"
            >
            <?php echo $info['note'];
            ?></textarea>
    </div>
