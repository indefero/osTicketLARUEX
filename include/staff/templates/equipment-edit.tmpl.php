...
if (!$info['title'])
    $info['title'] = sprintf(__('%s %s'),
            __('Edit'), $equipment->getName()
            );

$action = $info['action'] ?: ('#equipments/'.$equipment->getId().'/edit');

$namespace = sprintf('equipment.%d.edit', $equipment->getId());
...
<form method="post" class="task" action="<?php echo $action; ?>">
    ...
    <div>
        <textarea class="richtext no-bar" name="note" cols="21" rows="6"
            style="width:80%;"
            placeholder="<?php echo __('Reason for editing the equipment (optional)'); ?>"
            >
            <?php echo $info['note'];
            ?></textarea>
    </div>
