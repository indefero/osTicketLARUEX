<?php
// Task Schedule mass actions based on logged in agent
...
if ($actions && !isset($options['status'])) {
    ...
    <?php foreach ($actions as $a => $action) { ?>
            <li <?php
                ...
                <a class="no-pjax task-schedules-action"
                    <?php
                    if ($action['dialog'])
                    ...
                    ?>
                    href="<?php
                    echo sprintf('#task-schedule/mass/%s', $a); ?>"
 ...
 <?php
    ...
    <!--<span class="red button action-button">
     <a class="task-schedules-action" id="task-schedule-delete" data-placement="bottom"
...
$(function() {
    $(document).off('.task-schedules-actions');
    $(document).on('click.task-schedules-actions', 'a.task-schedules-action', function(e) {
        e.preventDefault();
        var $form = $('form#schedules');
