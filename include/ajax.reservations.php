...
/*********************************************************************
    ...
    AJAX interface for reservations
...
class ReservationsAjaxAPI extends AjaxController {
    ...
    function add() {
        ...
        $info=$errors=array();
        if ($_POST) {
            Draft::deleteForNamespace('reservation.add', $thisstaff->getId());
            // Default form
    ...
    function massProcess($action, $w=null)  {
        ...
        case 'delete':
            $inc = 'delete.tmpl.php';
            $info[':action'] = '#reservations/mass/delete';
            $info[':title'] = sprintf('Eliminar %s',
                    _N('la reserva seleccionada', 'las reservas seleccionadas', $count));
            ...
            $info[':extra'] = sprintf('<strong>%s</strong>',
                        __('SÓLO puede eliminar sus reservas.')
                        );
        ...
        if ($_POST && $i) {
            ...
            if ($i==$count) {
                ...
                $warn = sprintf(
                        ...
                        _N('reserva seleccionada', 'reservas seleccionadas', $count),
                        $actions[$action]['verbed']);
        ...
        $(function() {
            $('form#reservations input[name=\"tids[]\"]:checkbox:checked')
            .each(function() {
                $('<input>')
                .prop('type', 'hidden')
                .attr('name', 'tids[]')
