...
INSERT INTO `ost_session` VALUES ('2p974k45gieceekfg4m2sid651','csrf|a:2:{s:5:\"token\";s:40:\"28060ba70bf00bf7cd83f6c8ddbc3a98dbc447ff\";s:4:\"time\";i:1509608501;}_staff|a:1:{s:4:\"auth\";a:2:{s:4:\"dest\";s:53:\"/scp/logout.php?auth=31b46c08fde0d18ff7e2197d504904ce\";s:3:\"msg\";s:24:\"Autenticación Requerida\";}}_auth|a:1:{s:5:\"staff\";N;}','2017-11-03 08:41:41',NULL,'0','172.22.15.53','Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:56.0) Gecko/20100101 Firefox/56.0');
...
INSERT INTO `ost_syslog` VALUES (14,'Warning','Token CSRF inválido __CSRFToken__','Token CSRF inválido [5b982573306c98a996a9d59ea1d5eeb9eb696a0d] en http://172.22.13.78/scp/login.php','','172.22.15.53','2017-10-30 11:52:50','2017-10-30 11:52:50'),(15,'Warning','Token CSRF inválido __CSRFToken__','Token CSRF inválido [ded2590be99f77cf30467684d28db1c5a84e51f1] en http://172.22.13.78/scp/login.php','','172.22.15.53','2017-10-30 11:55:21','2017-10-30 11:55:21'),(16,'Error','DB Error #1241','[SELECT A1.`ticket_id`, A1.`number`, A1.`user_id`, A1.`user_email_id`, A1.`status_id`, A1.`dept_id`, A1.`sla_id`, A1.`topic_id`, A1.`staff_id`, A1.`team_id`, A1.`email_id`, A1.`lock_id`, A1.`flags`, A1.`ip_address`, A1.`source`, A1.`source_extra`, A1.`isoverdue`, A1.`isanswered`, A1.`duedate`, A1.`est_duedate`, A1.`reopened`, A1.`closed`, A1.`lastupdate`, A1.`created`, A1.`updated`, A1.`last_duedate`, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_collaborator` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id`) AS collab_count, (SELECT COUNT(R1.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) LEFT JOIN `ost_attachment` R1 ON (R1.`type` = \'H\' AND R0.`id` = R1.`object_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND R1.`inline` = 0) AS attachment_count, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND NOT R0.`flags` &amp; 4 != 0) AS thread_count, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'descripcion\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS descripcion, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'causas\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS causas, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'consecuencias\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS consecuencias, (SELECT Q7.* FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) WHERE Q8.`ticket_id` = A1.`ticket_id`) AS acciones, A2.`id`, A2.`org_id`, A2.`default_email_id`, A2.`status`, A2.`name`, A2.`created`, A2.`updated`, A3.`staff_id`, A3.`dept_id`, A3.`role_id`, A3.`username`, A3.`firstname`, A3.`lastname`, A3.`passwd`, A3.`backend`, A3.`email`, A3.`phone`, A3.`phone_ext`, A3.`mobile`, A3.`signature`, A3.`lang`, A3.`timezone`, A3.`locale`, A3.`notes`, A3.`isactive`, A3.`isadmin`, A3.`isvisible`, A3.`onvacation`, A3.`assigned_only`, A3.`show_assigned_tickets`, A3.`change_passwd`, A3.`max_page_size`, A3.`auto_refresh_rate`, A3.`default_signature_type`, A3.`default_paper_size`, A3.`extra`, A3.`permissions`, A3.`created`, A3.`lastlogin`, A3.`passwdreset`, A3.`updated`, A4.`team_id`, A4.`lead_id`, A4.`flags`, A4.`name`, A4.`notes`, A4.`created`, A4.`updated`, A5.`ticket_id`, A5.`subject`, A5.`priority`, A6.`id`, A6.`name`, A6.`state`, A6.`mode`, A6.`flags`, A6.`sort`, A6.`properties`, A6.`created`, A6.`updated`, A7.`priority_id`, A7.`priority`, A7.`priority_desc`, A7.`priority_color`, A7.`priority_urgency`, A7.`ispublic` FROM `ost_ticket` A1 JOIN `ost_user` A2 ON (A1.`user_id` = A2.`id`) LEFT JOIN `ost_staff` A3 ON (A1.`staff_id` = A3.`staff_id`) LEFT JOIN `ost_team` A4 ON (A1.`team_id` = A4.`team_id`) LEFT JOIN `ost_ticket__cdata` A5 ON (A1.`ticket_id` = A5.`ticket_id`) JOIN `ost_ticket_status` A6 ON (A1.`status_id` = A6.`id`) LEFT JOIN `ost_ticket_priority` A7 ON (A5.`priority` = A7.`priority_id`) WHERE A1.`ticket_id` IN (SELECT A1.`ticket_id` FROM `ost_ticket` A1 JOIN `ost_ticket_status` A2 ON (A1.`status_id` = A2.`id`) WHERE A2.`state` = \'closed\' AND ((A2.`state` = \'open\' AND A1.`staff_id` = 1) OR A1.`dept_id` IN (\'4\', \'5\', \'6\', \'7\', \'8\'))) GROUP BY A1.ticket_id ORDER BY A1.`closed` DESC] Operand should contain 1 column(s)<br /><br /> ---- Recular ----<br /> #0 (raíz)/include/mysqli.php(204): osTicket-&gt;logDBError(\'DB Error #1241\', \'[SELECT A1.`tic...\')<br /> #1 (raíz)/include/class.orm.php(3135): db_query(\'SELECT A1.`tick...\', true, false)<br /> #2 (raíz)/include/class.orm.php(3191): MySqlExecutor-&gt;execute()<br /> #3 (raíz)/include/class.orm.php(1773): MySqlExecutor-&gt;getRow()<br /> #4 (raíz)/include/class.orm.php(1817): ModelInstanceManager-&gt;{closure}()<br /> #5 (raíz)/include/class.orm.php(1796): CallbackSimpleIterator-&gt;next()<br /> #6 (raíz)/include/class.export.php(310): CallbackSimpleIterator-&gt;rewind()<br /> #7 (raíz)/include/class.export.php(32): ResultSetExporter-&gt;__construct(Object(QuerySet), Array, Array)<br /> #8 (raíz)/include/class.export.php(139): Export::dumpQuery(Object(QuerySet), Array, \'csv\', Array)<br /> #9 (raíz)/include/class.export.php(145): Export::dumpTickets(Object(QuerySet), \'csv\')<br /> #10 (raíz)/scp/tickets.php(478): Export::saveTickets(Object(QuerySet), \'tickets-2017103...\', \'csv\')<br /> #11 {main}','','172.22.15.53','2017-10-30 12:36:02','2017-10-30 12:36:02'),(17,'Error','DB Error #1241','[SELECT A1.`ticket_id`, A1.`number`, A1.`user_id`, A1.`user_email_id`, A1.`status_id`, A1.`dept_id`, A1.`sla_id`, A1.`topic_id`, A1.`staff_id`, A1.`team_id`, A1.`email_id`, A1.`lock_id`, A1.`flags`, A1.`ip_address`, A1.`source`, A1.`source_extra`, A1.`isoverdue`, A1.`isanswered`, A1.`duedate`, A1.`est_duedate`, A1.`reopened`, A1.`closed`, A1.`lastupdate`, A1.`created`, A1.`updated`, A1.`last_duedate`, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_collaborator` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id`) AS collab_count, (SELECT COUNT(R1.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) LEFT JOIN `ost_attachment` R1 ON (R1.`type` = \'H\' AND R0.`id` = R1.`object_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND R1.`inline` = 0) AS attachment_count, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND NOT R0.`flags` &amp; 4 != 0) AS thread_count, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'descripcion\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS descripcion, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'causas\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS causas, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'consecuencias\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS consecuencias, (SELECT Q7.* FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` ORDER BY R0.`created` ASC) AS acciones, A2.`id`, A2.`org_id`, A2.`default_email_id`, A2.`status`, A2.`name`, A2.`created`, A2.`updated`, A3.`staff_id`, A3.`dept_id`, A3.`role_id`, A3.`username`, A3.`firstname`, A3.`lastname`, A3.`passwd`, A3.`backend`, A3.`email`, A3.`phone`, A3.`phone_ext`, A3.`mobile`, A3.`signature`, A3.`lang`, A3.`timezone`, A3.`locale`, A3.`notes`, A3.`isactive`, A3.`isadmin`, A3.`isvisible`, A3.`onvacation`, A3.`assigned_only`, A3.`show_assigned_tickets`, A3.`change_passwd`, A3.`max_page_size`, A3.`auto_refresh_rate`, A3.`default_signature_type`, A3.`default_paper_size`, A3.`extra`, A3.`permissions`, A3.`created`, A3.`lastlogin`, A3.`passwdreset`, A3.`updated`, A4.`team_id`, A4.`lead_id`, A4.`flags`, A4.`name`, A4.`notes`, A4.`created`, A4.`updated`, A5.`ticket_id`, A5.`subject`, A5.`priority`, A6.`id`, A6.`name`, A6.`state`, A6.`mode`, A6.`flags`, A6.`sort`, A6.`properties`, A6.`created`, A6.`updated`, A7.`priority_id`, A7.`priority`, A7.`priority_desc`, A7.`priority_color`, A7.`priority_urgency`, A7.`ispublic` FROM `ost_ticket` A1 JOIN `ost_user` A2 ON (A1.`user_id` = A2.`id`) LEFT JOIN `ost_staff` A3 ON (A1.`staff_id` = A3.`staff_id`) LEFT JOIN `ost_team` A4 ON (A1.`team_id` = A4.`team_id`) LEFT JOIN `ost_ticket__cdata` A5 ON (A1.`ticket_id` = A5.`ticket_id`) JOIN `ost_ticket_status` A6 ON (A1.`status_id` = A6.`id`) LEFT JOIN `ost_ticket_priority` A7 ON (A5.`priority` = A7.`priority_id`) WHERE A1.`ticket_id` IN (SELECT A1.`ticket_id` FROM `ost_ticket` A1 JOIN `ost_ticket_status` A2 ON (A1.`status_id` = A2.`id`) WHERE A2.`state` = \'closed\' AND ((A2.`state` = \'open\' AND A1.`staff_id` = 1) OR A1.`dept_id` IN (\'4\', \'5\', \'6\', \'7\', \'8\'))) GROUP BY A1.ticket_id ORDER BY A1.`closed` DESC] Operand should contain 1 column(s)<br /><br /> ---- Recular ----<br /> #0 (raíz)/include/mysqli.php(204): osTicket-&gt;logDBError(\'DB Error #1241\', \'[SELECT A1.`tic...\')<br /> #1 (raíz)/include/class.orm.php(3135): db_query(\'SELECT A1.`tick...\', true, false)<br /> #2 (raíz)/include/class.orm.php(3191): MySqlExecutor-&gt;execute()<br /> #3 (raíz)/include/class.orm.php(1773): MySqlExecutor-&gt;getRow()<br /> #4 (raíz)/include/class.orm.php(1817): ModelInstanceManager-&gt;{closure}()<br /> #5 (raíz)/include/class.orm.php(1796): CallbackSimpleIterator-&gt;next()<br /> #6 (raíz)/include/class.export.php(310): CallbackSimpleIterator-&gt;rewind()<br /> #7 (raíz)/include/class.export.php(32): ResultSetExporter-&gt;__construct(Object(QuerySet), Array, Array)<br /> #8 (raíz)/include/class.export.php(139): Export::dumpQuery(Object(QuerySet), Array, \'csv\', Array)<br /> #9 (raíz)/include/class.export.php(145): Export::dumpTickets(Object(QuerySet), \'csv\')<br /> #10 (raíz)/scp/tickets.php(478): Export::saveTickets(Object(QuerySet), \'tickets-2017103...\', \'csv\')<br /> #11 {main}','','172.22.15.53','2017-10-30 12:37:45','2017-10-30 12:37:45'),(18,'Error','DB Error #1241','[SELECT A1.`ticket_id`, A1.`number`, A1.`user_id`, A1.`user_email_id`, A1.`status_id`, A1.`dept_id`, A1.`sla_id`, A1.`topic_id`, A1.`staff_id`, A1.`team_id`, A1.`email_id`, A1.`lock_id`, A1.`flags`, A1.`ip_address`, A1.`source`, A1.`source_extra`, A1.`isoverdue`, A1.`isanswered`, A1.`duedate`, A1.`est_duedate`, A1.`reopened`, A1.`closed`, A1.`lastupdate`, A1.`created`, A1.`updated`, A1.`last_duedate`, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_collaborator` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id`) AS collab_count, (SELECT COUNT(R1.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) LEFT JOIN `ost_attachment` R1 ON (R1.`type` = \'H\' AND R0.`id` = R1.`object_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND R1.`inline` = 0) AS attachment_count, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND NOT R0.`flags` &amp; 4 != 0) AS thread_count, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'descripcion\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS descripcion, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'causas\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS causas, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'consecuencias\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS consecuencias, (SELECT Q7.`id`, Q7.`pid`, Q7.`thread_id`, Q7.`staff_id`, Q7.`user_id`, Q7.`type`, Q7.`flags`, Q7.`poster`, Q7.`editor`, Q7.`editor_type`, Q7.`source`, Q7.`title`, Q7.`body`, Q7.`format`, Q7.`ip_address`, Q7.`created`, Q7.`updated`, R1.`staff_id`, R1.`dept_id`, R1.`role_id`, R1.`username`, R1.`firstname`, R1.`lastname`, R1.`passwd`, R1.`backend`, R1.`email`, R1.`phone`, R1.`phone_ext`, R1.`mobile`, R1.`signature`, R1.`lang`, R1.`timezone`, R1.`locale`, R1.`notes`, R1.`isactive`, R1.`isadmin`, R1.`isvisible`, R1.`onvacation`, R1.`assigned_only`, R1.`show_assigned_tickets`, R1.`change_passwd`, R1.`max_page_size`, R1.`auto_refresh_rate`, R1.`default_signature_type`, R1.`default_paper_size`, R1.`extra`, R1.`permissions`, R1.`created`, R1.`lastlogin`, R1.`passwdreset`, R1.`updated`, R2.`id`, R2.`org_id`, R2.`default_email_id`, R2.`status`, R2.`name`, R2.`created`, R2.`updated`, R3.`id`, R3.`thread_entry_id`, R3.`mid`, R3.`headers` FROM `ost_thread_entry` Q7 JOIN `ost_thread` Q8 ON (Q7.`thread_id` = Q8.`id`) JOIN `ost_ticket` R0 ON (Q8.`object_type` = \'T\' AND Q8.`object_id` = R0.`ticket_id`) LEFT JOIN `ost_staff` R1 ON (Q7.`staff_id` = R1.`staff_id`) LEFT JOIN `ost_user` R2 ON (Q7.`user_id` = R2.`id`) LEFT JOIN `ost_thread_entry_email` R3 ON (Q7.`id` = R3.`thread_entry_id`) WHERE R0.`ticket_id` = A1.`ticket_id` ORDER BY Q7.`created` DESC) AS acciones, A2.`id`, A2.`org_id`, A2.`default_email_id`, A2.`status`, A2.`name`, A2.`created`, A2.`updated`, A3.`staff_id`, A3.`dept_id`, A3.`role_id`, A3.`username`, A3.`firstname`, A3.`lastname`, A3.`passwd`, A3.`backend`, A3.`email`, A3.`phone`, A3.`phone_ext`, A3.`mobile`, A3.`signature`, A3.`lang`, A3.`timezone`, A3.`locale`, A3.`notes`, A3.`isactive`, A3.`isadmin`, A3.`isvisible`, A3.`onvacation`, A3.`assigned_only`, A3.`show_assigned_tickets`, A3.`change_passwd`, A3.`max_page_size`, A3.`auto_refresh_rate`, A3.`default_signature_type`, A3.`default_paper_size`, A3.`extra`, A3.`permissions`, A3.`created`, A3.`lastlogin`, A3.`passwdreset`, A3.`updated`, A4.`team_id`, A4.`lead_id`, A4.`flags`, A4.`name`, A4.`notes`, A4.`created`, A4.`updated`, A5.`ticket_id`, A5.`subject`, A5.`priority`, A6.`id`, A6.`name`, A6.`state`, A6.`mode`, A6.`flags`, A6.`sort`, A6.`properties`, A6.`created`, A6.`updated`, A7.`priority_id`, A7.`priority`, A7.`priority_desc`, A7.`priority_color`, A7.`priority_urgency`, A7.`ispublic` FROM `ost_ticket` A1 JOIN `ost_user` A2 ON (A1.`user_id` = A2.`id`) LEFT JOIN `ost_staff` A3 ON (A1.`staff_id` = A3.`staff_id`) LEFT JOIN `ost_team` A4 ON (A1.`team_id` = A4.`team_id`) LEFT JOIN `ost_ticket__cdata` A5 ON (A1.`ticket_id` = A5.`ticket_id`) JOIN `ost_ticket_status` A6 ON (A1.`status_id` = A6.`id`) LEFT JOIN `ost_ticket_priority` A7 ON (A5.`priority` = A7.`priority_id`) WHERE A1.`ticket_id` IN (SELECT A1.`ticket_id` FROM `ost_ticket` A1 JOIN `ost_ticket_status` A2 ON (A1.`status_id` = A2.`id`) WHERE A2.`state` = \'closed\' AND ((A2.`state` = \'open\' AND A1.`staff_id` = 1) OR A1.`dept_id` IN (\'4\', \'5\', \'6\', \'7\', \'8\'))) GROUP BY A1.ticket_id ORDER BY A1.`closed` DESC] Operand should contain 1 column(s)<br /><br /> ---- Recular ----<br /> #0 (raíz)/include/mysqli.php(204): osTicket-&gt;logDBError(\'DB Error #1241\', \'[SELECT A1.`tic...\')<br /> #1 (raíz)/include/class.orm.php(3135): db_query(\'SELECT A1.`tick...\', true, false)<br /> #2 (raíz)/include/class.orm.php(3191): MySqlExecutor-&gt;execute()<br /> #3 (raíz)/include/class.orm.php(1773): MySqlExecutor-&gt;getRow()<br /> #4 (raíz)/include/class.orm.php(1817): ModelInstanceManager-&gt;{closure}()<br /> #5 (raíz)/include/class.orm.php(1796): CallbackSimpleIterator-&gt;next()<br /> #6 (raíz)/include/class.export.php(310): CallbackSimpleIterator-&gt;rewind()<br /> #7 (raíz)/include/class.export.php(32): ResultSetExporter-&gt;__construct(Object(QuerySet), Array, Array)<br /> #8 (raíz)/include/class.export.php(139): Export::dumpQuery(Object(QuerySet), Array, \'csv\', Array)<br /> #9 (raíz)/include/class.export.php(145): Export::dumpTickets(Object(QuerySet), \'csv\')<br /> #10 (raíz)/scp/tickets.php(478): Export::saveTickets(Object(QuerySet), \'tickets-2017103...\', \'csv\')<br /> #11 {main}','','172.22.15.53','2017-10-30 12:49:57','2017-10-30 12:49:57'),(19,'Error','DB Error #1241','[SELECT A1.`ticket_id`, A1.`number`, A1.`user_id`, A1.`user_email_id`, A1.`status_id`, A1.`dept_id`, A1.`sla_id`, A1.`topic_id`, A1.`staff_id`, A1.`team_id`, A1.`email_id`, A1.`lock_id`, A1.`flags`, A1.`ip_address`, A1.`source`, A1.`source_extra`, A1.`isoverdue`, A1.`isanswered`, A1.`duedate`, A1.`est_duedate`, A1.`reopened`, A1.`closed`, A1.`lastupdate`, A1.`created`, A1.`updated`, A1.`last_duedate`, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_collaborator` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id`) AS collab_count, (SELECT COUNT(R1.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) LEFT JOIN `ost_attachment` R1 ON (R1.`type` = \'H\' AND R0.`id` = R1.`object_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND R1.`inline` = 0) AS attachment_count, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND NOT R0.`flags` &amp; 4 != 0) AS thread_count, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'descripcion\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS descripcion, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'causas\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS causas, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'consecuencias\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS consecuencias, (SELECT Q7.* FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` ORDER BY R0.`created` DESC) AS acciones, A2.`id`, A2.`org_id`, A2.`default_email_id`, A2.`status`, A2.`name`, A2.`created`, A2.`updated`, A3.`staff_id`, A3.`dept_id`, A3.`role_id`, A3.`username`, A3.`firstname`, A3.`lastname`, A3.`passwd`, A3.`backend`, A3.`email`, A3.`phone`, A3.`phone_ext`, A3.`mobile`, A3.`signature`, A3.`lang`, A3.`timezone`, A3.`locale`, A3.`notes`, A3.`isactive`, A3.`isadmin`, A3.`isvisible`, A3.`onvacation`, A3.`assigned_only`, A3.`show_assigned_tickets`, A3.`change_passwd`, A3.`max_page_size`, A3.`auto_refresh_rate`, A3.`default_signature_type`, A3.`default_paper_size`, A3.`extra`, A3.`permissions`, A3.`created`, A3.`lastlogin`, A3.`passwdreset`, A3.`updated`, A4.`team_id`, A4.`lead_id`, A4.`flags`, A4.`name`, A4.`notes`, A4.`created`, A4.`updated`, A5.`ticket_id`, A5.`subject`, A5.`priority`, A6.`id`, A6.`name`, A6.`state`, A6.`mode`, A6.`flags`, A6.`sort`, A6.`properties`, A6.`created`, A6.`updated`, A7.`priority_id`, A7.`priority`, A7.`priority_desc`, A7.`priority_color`, A7.`priority_urgency`, A7.`ispublic` FROM `ost_ticket` A1 JOIN `ost_user` A2 ON (A1.`user_id` = A2.`id`) LEFT JOIN `ost_staff` A3 ON (A1.`staff_id` = A3.`staff_id`) LEFT JOIN `ost_team` A4 ON (A1.`team_id` = A4.`team_id`) LEFT JOIN `ost_ticket__cdata` A5 ON (A1.`ticket_id` = A5.`ticket_id`) JOIN `ost_ticket_status` A6 ON (A1.`status_id` = A6.`id`) LEFT JOIN `ost_ticket_priority` A7 ON (A5.`priority` = A7.`priority_id`) WHERE A1.`ticket_id` IN (SELECT A1.`ticket_id` FROM `ost_ticket` A1 JOIN `ost_ticket_status` A2 ON (A1.`status_id` = A2.`id`) WHERE A2.`state` = \'closed\' AND ((A2.`state` = \'open\' AND A1.`staff_id` = 1) OR A1.`dept_id` IN (\'4\', \'5\', \'6\', \'7\', \'8\'))) GROUP BY A1.ticket_id ORDER BY A1.`closed` DESC] Operand should contain 1 column(s)<br /><br /> ---- Recular ----<br /> #0 (raíz)/include/mysqli.php(204): osTicket-&gt;logDBError(\'DB Error #1241\', \'[SELECT A1.`tic...\')<br /> #1 (raíz)/include/class.orm.php(3135): db_query(\'SELECT A1.`tick...\', true, false)<br /> #2 (raíz)/include/class.orm.php(3191): MySqlExecutor-&gt;execute()<br /> #3 (raíz)/include/class.orm.php(1773): MySqlExecutor-&gt;getRow()<br /> #4 (raíz)/include/class.orm.php(1817): ModelInstanceManager-&gt;{closure}()<br /> #5 (raíz)/include/class.orm.php(1796): CallbackSimpleIterator-&gt;next()<br /> #6 (raíz)/include/class.export.php(310): CallbackSimpleIterator-&gt;rewind()<br /> #7 (raíz)/include/class.export.php(32): ResultSetExporter-&gt;__construct(Object(QuerySet), Array, Array)<br /> #8 (raíz)/include/class.export.php(139): Export::dumpQuery(Object(QuerySet), Array, \'csv\', Array)<br /> #9 (raíz)/include/class.export.php(145): Export::dumpTickets(Object(QuerySet), \'csv\')<br /> #10 (raíz)/scp/tickets.php(478): Export::saveTickets(Object(QuerySet), \'tickets-2017103...\', \'csv\')<br /> #11 {main}','','172.22.15.53','2017-10-30 12:53:06','2017-10-30 12:53:06'),(20,'Error','DB Error #1054','[SELECT A1.`ticket_id`, A1.`number`, A1.`user_id`, A1.`user_email_id`, A1.`status_id`, A1.`dept_id`, A1.`sla_id`, A1.`topic_id`, A1.`staff_id`, A1.`team_id`, A1.`email_id`, A1.`lock_id`, A1.`flags`, A1.`ip_address`, A1.`source`, A1.`source_extra`, A1.`isoverdue`, A1.`isanswered`, A1.`duedate`, A1.`est_duedate`, A1.`reopened`, A1.`closed`, A1.`lastupdate`, A1.`created`, A1.`updated`, A1.`last_duedate`, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_collaborator` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id`) AS collab_count, (SELECT COUNT(R1.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) LEFT JOIN `ost_attachment` R1 ON (R1.`type` = \'H\' AND R0.`id` = R1.`object_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND R1.`inline` = 0) AS attachment_count, (SELECT COUNT(R0.`id`) AS `count` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` AND NOT R0.`flags` &amp; 4 != 0) AS thread_count, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'descripcion\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS descripcion, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'causas\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS causas, (SELECT Q8.`value` AS `answers__value` FROM `ost_form_entry` Q7 LEFT JOIN `ost_form_entry_values` Q8 ON (Q7.`id` = Q8.`entry_id`) LEFT JOIN `ost_form_field` R0 ON (Q8.`field_id` = R0.`id`) LEFT JOIN `ost_form` R1 ON (Q7.`form_id` = R1.`id`) WHERE (R0.`name` = \'consecuencias\' AND Q7.`object_id` = A1.`ticket_id` AND Q7.`object_type` = \'T\' AND R1.`title` = \'Notificación y cierre de desviaciones\') ORDER BY Q7.`sort` ASC) AS consecuencias, (SELECT R0.`bodyasdas` AS `entries__bodyasdas` FROM `ost_thread` Q7 JOIN `ost_ticket` Q8 ON (Q7.`object_type` = \'T\' AND Q7.`object_id` = Q8.`ticket_id`) LEFT JOIN `ost_thread_entry` R0 ON (Q7.`id` = R0.`thread_id`) WHERE Q8.`ticket_id` = A1.`ticket_id` ORDER BY R0.`created` DESC) AS acciones, A2.`id`, A2.`org_id`, A2.`default_email_id`, A2.`status`, A2.`name`, A2.`created`, A2.`updated`, A3.`staff_id`, A3.`dept_id`, A3.`role_id`, A3.`username`, A3.`firstname`, A3.`lastname`, A3.`passwd`, A3.`backend`, A3.`email`, A3.`phone`, A3.`phone_ext`, A3.`mobile`, A3.`signature`, A3.`lang`, A3.`timezone`, A3.`locale`, A3.`notes`, A3.`isactive`, A3.`isadmin`, A3.`isvisible`, A3.`onvacation`, A3.`assigned_only`, A3.`show_assigned_tickets`, A3.`change_passwd`, A3.`max_page_size`, A3.`auto_refresh_rate`, A3.`default_signature_type`, A3.`default_paper_size`, A3.`extra`, A3.`permissions`, A3.`created`, A3.`lastlogin`, A3.`passwdreset`, A3.`updated`, A4.`team_id`, A4.`lead_id`, A4.`flags`, A4.`name`, A4.`notes`, A4.`created`, A4.`updated`, A5.`ticket_id`, A5.`subject`, A5.`priority`, A6.`id`, A6.`name`, A6.`state`, A6.`mode`, A6.`flags`, A6.`sort`, A6.`properties`, A6.`created`, A6.`updated`, A7.`priority_id`, A7.`priority`, A7.`priority_desc`, A7.`priority_color`, A7.`priority_urgency`, A7.`ispublic` FROM `ost_ticket` A1 JOIN `ost_user` A2 ON (A1.`user_id` = A2.`id`) LEFT JOIN `ost_staff` A3 ON (A1.`staff_id` = A3.`staff_id`) LEFT JOIN `ost_team` A4 ON (A1.`team_id` = A4.`team_id`) LEFT JOIN `ost_ticket__cdata` A5 ON (A1.`ticket_id` = A5.`ticket_id`) JOIN `ost_ticket_status` A6 ON (A1.`status_id` = A6.`id`) LEFT JOIN `ost_ticket_priority` A7 ON (A5.`priority` = A7.`priority_id`) WHERE A1.`ticket_id` IN (SELECT A1.`ticket_id` FROM `ost_ticket` A1 JOIN `ost_ticket_status` A2 ON (A1.`status_id` = A2.`id`) WHERE A2.`state` = \'closed\' AND ((A2.`state` = \'open\' AND A1.`staff_id` = 1) OR A1.`dept_id` IN (\'4\', \'5\', \'6\', \'7\', \'8\'))) GROUP BY A1.ticket_id ORDER BY A1.`closed` DESC] Unknown column \'R0.bodyasdas\' in \'field list\'<br /><br /> ---- Recular ----<br /> #0 (raíz)/include/mysqli.php(204): osTicket-&gt;logDBError(\'DB Error #1054\', \'[SELECT A1.`tic...\')<br /> #1 (raíz)/include/class.orm.php(3135): db_query(\'SELECT A1.`tick...\', true, false)<br /> #2 (raíz)/include/class.orm.php(3191): MySqlExecutor-&gt;execute()<br /> #3 (raíz)/include/class.orm.php(1773): MySqlExecutor-&gt;getRow()<br /> #4 (raíz)/include/class.orm.php(1817): ModelInstanceManager-&gt;{closure}()<br /> #5 (raíz)/include/class.orm.php(1796): CallbackSimpleIterator-&gt;next()<br /> #6 (raíz)/include/class.export.php(310): CallbackSimpleIterator-&gt;rewind()<br /> #7 (raíz)/include/class.export.php(32): ResultSetExporter-&gt;__construct(Object(QuerySet), Array, Array)<br /> #8 (raíz)/include/class.export.php(139): Export::dumpQuery(Object(QuerySet), Array, \'csv\', Array)<br /> #9 (raíz)/include/class.export.php(145): Export::dumpTickets(Object(QuerySet), \'csv\')<br /> #10 (raíz)/scp/tickets.php(478): Export::saveTickets(Object(QuerySet), \'tickets-2017103...\', \'csv\')<br /> #11 {main}','','172.22.15.53','2017-10-30 12:55:14','2017-10-30 12:55:14'),(21,'Warning','Error de plantilla de recuperación','Incapaz de recuperar la plantilla \"ticket.solved\" - id#0','','172.22.15.53','2017-10-31 09:51:15','2017-10-31 09:51:15'),(22,'Warning','Error de plantilla de recuperación','Incapaz de recuperar la plantilla \"ticket.solved\" - id#0','','172.22.15.53','2017-10-31 10:59:27','2017-10-31 10:59:27'),(23,'Warning','Error de plantilla de recuperación','Incapaz de recuperar la plantilla \"ticket.solved\" - id#0','','172.22.15.53','2017-10-31 11:06:28','2017-10-31 11:06:28'),(24,'Warning','Error de plantilla de recuperación','Incapaz de recuperar la plantilla \"ticket.solved\" - id#0','','172.22.15.53','2017-10-31 11:12:16','2017-10-31 11:12:16'),(25,'Warning','Error de plantilla de recuperación','Incapaz de recuperar la plantilla \"ticket.solved\" - id#0','','172.22.15.53','2017-10-31 11:20:38','2017-10-31 11:20:38');
...
-- Dump completed on 2017-11-02  8:41:44
