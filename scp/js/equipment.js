/*********************************************************************
    ...
    Equipment utility loaded on equipment view!

    Useful for UI config settings, equipment locking ...etc
  ...
  Lock.prototype = {
    ...
    acquire: function() {
      ...
      this.ajaxActive = $.ajax({
        type: "POST",
        url: 'ajax.php/equipmentlock/'+this.objectId,
        dataType: 'json',
        cache: false,
    ...
    renew: function() {
      ...
      this.ajaxActive = $.ajax({
        type: "POST",
        url: 'ajax.php/equipmentlock/{0}/{1}/renew'.replace('{0}',this.lockId).replace('{1}',this.objectId),
        dataType: 'json',
        cache: false,
        success: $.proxy(this.update, this),
    ...
    release: function() {
      ...
      $.ajax({
        type: 'POST',
        url: 'ajax.php/equipmentlock/{0}/release'.replace('{0}', this.lockId),
        data: 'delete',
        cache: false,
    ...
    warn: function() {
      $.messageBar.show(
        ...
        __('The lock you hold on this equipment will expire soon. Would you like to renew the lock?'),
    ...
    fail: function(msg) {
      ...
      this.errorBar = $.messageBar.show(
        msg || __('Unable to lock the equipment.'),
        __('Someone else could be working on the same equipment.'),
...
$.refreshEquipmentView = function(interval) {
...
var equipment_onload = function($) {
    if (0 === $('#equipmentThread').length)
        return;
    ...
    $('a#equipment-print').click(function(e) {
        e.preventDefault();
        $('#overlay').show();
...
$(equipment_onload);
$(document).on('pjax:success', function() { equipment_onload(jQuery); });
